...
def to_cuda(data):
    ...
    def _set_graph_on_cuda(graph):
        for key_graph, val_graph in graph.ndata.items():
            # print('ndata key_graph', key_graph)
            # print('ndata val_graph', val_graph)
            # print('ndata val_graph.shape', val_graph.shape)
            graph.ndata[key_graph] = graph.ndata.pop(key_graph).cuda()
        for key_graph, val_graph in graph.edata.items():
            # print('edata key_graph', key_graph)
            # print('edata val_graph', val_graph)
            # print('edata val_graph.shape', val_graph.shape)
